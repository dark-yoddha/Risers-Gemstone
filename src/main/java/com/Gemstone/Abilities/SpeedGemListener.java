...
public class SpeedGemListener implements Listener {
    ...
    private boolean isSpeedGem(ItemStack item) {
        return item != null && item.getType() == Material.LIGHT_BLUE_DYE && item.hasItemMeta() &&
                item.getItemMeta().getDisplayName().equals(String.valueOf(ChatColor.DARK_AQUA) + "Speed Gem");
    }
}